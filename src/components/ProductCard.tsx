...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  const { name, description, price, discount, paymentOptions, image, colors } =
    product;
    ...
    >
      ...
      <CardContent>
        ...
        {discount > 0 && (
          <Typography
            variant="body1"
            sx={{ textDecoration: "line-through", color: "#313926" }}
          >
            R$ {price.toFixed(2)}
          </Typography>
        )}
        ...
        <div style={{ color: "#313926", marginTop: "10px" }}>
          {paymentOptions && paymentOptions.length > 0 ? (
            ...
            <Typography variant="caption">
              Nenhuma opção de pagamento disponível
            </Typography>
          )}
        </div>

        {/* Exibindo as opções de cores */}
        <Box
          sx={{
            marginTop: "10px",
            display: "flex",
